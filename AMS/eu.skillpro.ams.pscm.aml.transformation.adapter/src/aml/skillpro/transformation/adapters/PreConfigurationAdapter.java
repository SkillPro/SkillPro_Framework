...
import aml.skillpro.transformation.interfaces.IPreConfigurationTransformable;
...
public class PreConfigurationAdapter extends TransformableAdapterTemplate implements IPreConfigurationTransformable {
	@Override
	public ITransformable transform(Hierarchy<InternalElement> object,
		...
		if (object.getChildren() != null && object.getChildren().size() > 1) {
			throw new IllegalArgumentException("Pre-Configuration size should be 1 for this ResourceSkill: " + resourceSkill);
		}
		for (Hierarchy<InternalElement> child : object.getChildren()) {
			...
			resourceSkill.setPreConfiguration((ResourceConfiguration) transformedConf.getElement());
		}
