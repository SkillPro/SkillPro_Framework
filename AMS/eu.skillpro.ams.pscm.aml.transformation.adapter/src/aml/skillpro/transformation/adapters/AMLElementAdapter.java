...
public class AMLElementAdapter extends TransformableAdapterTemplate implements IAMLElementTransformable {
	@Override
	...
	public ITransformable transform(Hierarchy<InternalElement> object,
		...
		Map<Object, ITransformable> transformedObjectsMap = AMLTransformationService.getTransformationProvider()
				.getTransformationRepo().getTransformedObjectsMap();
		Map<Object, Class<? extends ITransformable>> adapterTransformablesMapping = 
		...
		if (context.contains(parent)) {
			Class<? extends ITransformable> transClass = adapterTransformablesMapping.get(parent);
			if (IFactoryNodeTransformable.class.isAssignableFrom(transClass)) {
				ITransformable owner = ((IFactoryNodeTransformable) transClass.newInstance())
				.transform(parent, context);
				((FactoryNode) owner.getElement()).addAMLElement(parent);
			} else if (IResourceSkillTransformable.class.isAssignableFrom(transClass)) {
				ITransformable owner = ((IResourceSkillTransformable) transClass.newInstance())
						.transform(parent, context);
				((ResourceSkill) owner.getElement()).addAMLElement(parent);
			} else if (IProductionSkillTransformable.class.isAssignableFrom(transClass)) {
				ITransformable owner = ((IProductionSkillTransformable) transClass.newInstance())
						.transform(parent, context);
				((ProductionSkill) owner.getElement()).addAMLElement(parent);
			} else {
