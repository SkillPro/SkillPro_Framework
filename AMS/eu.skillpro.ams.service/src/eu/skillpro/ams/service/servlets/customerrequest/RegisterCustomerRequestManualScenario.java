...
@WebServlet("/startScenario")
public class RegisterCustomerRequestManualScenario extends BaseServlet{
	private static final Logger logger = LoggerFactory.getLogger(RegisterCustomerRequestManualScenario.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		respondHTML(response, loadFile("startScenario.html"));

		try {
			ScenarioType scenarioType;
			int count;
			if (request.getParameter("ur5") != null) {
				scenarioType = ScenarioType.UR5_ONLY;
				count = p.getInt("ur5count");
			} else if (request.getParameter("kr6") != null) {
				scenarioType = ScenarioType.KR6_ONLY;
				count = p.getInt("kr6count");
			} else {
