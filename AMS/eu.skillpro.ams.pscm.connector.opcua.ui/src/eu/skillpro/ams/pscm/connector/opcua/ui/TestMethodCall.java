...
public class TestMethodCall extends AbstractHandler implements IHandler {
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			OPCUAServerRepository.registerSEE("xxx", "ttestsnippet", eu.skillpro.ams.pscm.connector.opcua.Activator.getDefault()
					.getCurrentUAaddress());
		} catch (SkillProOPCUAException e) {
			MessageDialog.openError(HandlerUtil.getActiveShell(event),
					"Error calling method: ",e.getMessage()+"\n"+"Beginning of the error trace:\n"+e.getStackTrace()[0].toString());
			e.printStackTrace();
		}
		return null;
	}
